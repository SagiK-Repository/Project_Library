...
BOOL CDlg::OnInitDialog()
{
	...
	//VTK
	InitializeVTKWindow(GetDlgItem(IDC_PIC2)->GetSafeHwnd());
	ResizeVTKWindow();
...
void CDlg::OnBnClickedButton5() // Distortion
{
	...
	Mat Re = Image01.clone();
	Re.convertTo(Re, CV_32F);
	resize(Re, Re, Size(500, 500));

	vtkNew<vtkNamedColors> colors;

	CRect rc;
	GetDlgItem(IDC_PIC2)->GetClientRect(rc);

	vtkNew<vtkChartXYZ> chart;
	chart->SetGeometry(vtkRectf(10.0, 10.0, rc.Width(), rc.Height())); //크기 및 위치
	...
	view->GetRenderWindow()->SetSize(rc.Width(), rc.Height());
	...
	for (vtkIdType i = 0; i < numPoints; ++i)
	{
		...
		for (vtkIdType j = 0; j < numPoints; ++j)
		{
			...
			table->SetValue(i, j, Re.at<float>(j, i*3) );
		}
	}
...
void CDlg::InitializeVTKWindow(void* hWnd)
{
